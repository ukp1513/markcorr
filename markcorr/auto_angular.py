...
def do_compute(realTab, realProperties, randTab, thMin, thNBins, thBinWidth, doRanking=True, 
               realRaCol='RA',realDecCol='DEC',randRaCol='RA', randDecCol='Dec', doBoot=False):
    ...
    th, omega = omega_theta(raReal, decReal, raRand, decRand, thMin, thNBins, thBinWidth)
    ...
    if len(realProperties) >= 1:
        ...
        for prop_i in realProperties:
            ...
            th, weightedOmega = weighted_omega_theta(raReal, decReal, weightReal, raRand, decRand, thMin, thNBins, thBinWidth)

            MThetaArray = np.array(mcf_theta(th, omega, weightedOmega)).reshape(len(th), 1)
